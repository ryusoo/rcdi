		...
		$(document).ready(function() {
			...
			$("#name").blur(function() {
				...
				if (name == "" || name.length == 0) {
					...
					$(".error_next_box").eq(0).text("이름은 표준 한글만  입력가능합니다.").css("display", "block").css("color", "#FF3636");
					return false;
				} else if (name.length > 4 || name.length < 2) {
					$(".error_next_box").eq(0).text("이름은 2자이상 4자 이하여야 합니다.").css("display","block").css("color", "#FF3636");
					return false;
				} else {
					$(".error_next_box").eq(0).text("멋진 이름이네요.").css("display", "block").css("color", "#0000FF");
				}
			});
			...
			$("#phone").blur(function() {
				...
				if (phone == "" || phone.length == 0) {
					...
					$(".error_next_box").eq(1).text("숫자만 입력해주세요.").css("display", "block").css("color", "#FF3636");
					return false;
					...
					// != 0 이기 때문에 조건문을 탄다
					$(".error_next_box").eq(1).text("휴대폰 번호가 유효하지 않습니다.").css("display", "block").css("color", "#FF3636");
					return false;
				} else if (!(phone.length == 10 || phone.length == 11)) {
					$(".error_next_box").eq(1).text("(-)없이 10자 또는 11자로 입력해주세요.").css("display","block").css("color", "#FF3636");
					return false;
				} else {
					$(".error_next_box").eq(1).text("등록가능한 휴대폰 번호입니다.").css("display", "block").css("color", "#0000FF");
				}
			});
			...
			$("#email_id").blur(function() {
				...
				if (email == "" || email.length == 0) {
					...
					if (!regEmail.test(fullMail)) {
						$(".error_next_box").eq(2).text("올바른 email을 입력해주세요.").css("display", "block").css("color","#FF3636");
						return false;
					} else {
						$(".error_next_box").eq(2).text("샤이니한 email이네요.").css("display","block").css("color", "#0000FF");
					}
				}
			});
